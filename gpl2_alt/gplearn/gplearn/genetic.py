...
from .functions import _function_map, _Function, add2,mul2,sig1 as sigmoid
...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    for i in range(n_programs):
        ...
        #denominator = (y_pred-np.average(y_pred))**2
        #denominator = np.where(denominator==0,1e-5,denominator)
        #b = np.sum(((y_pred-np.average(y_pred))*(y-np.average(y)))/denominator)
        #a = np.average(y) - b*np.average(y_pred)
        clf = Ridge(alpha=1.0)
        clf.fit(y_pred.reshape(-1,1), y.reshape(-1,1))
        b = (clf.coef_).reshape((-1,))[0]
        a = clf.intercept_[0]
        #print(a,b)
        program.program.insert(0,mul2)
        program.program.insert(0,add2)
        program.program.append(b)
        program.program.append(a)
