...
def _protected_division(left, right):
    res = np.where(right > 0.001, np.divide(left, right), 1e7)
    res = np.where(right > 1e7, 1e7, res)
    res = np.where(right < -1e7, -1e7, res)
    return res

def _protected_inverse(right):
    res = np.where(right > 0.001, np.divide(1, right), 1e7)
    res = np.where(right > 1e7, 1e7, res)
    res = np.where(right < -1e7, -1e7, res)
    return res
...
def _protected_log(arg):
    return np.where(abs(arg) < 1e-5, 1e-5, np.log(abs(arg))) 
    

def _protected_exponential(arg):
    return np.where(arg > 10, 10, np.exp(arg))
