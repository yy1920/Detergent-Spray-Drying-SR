...
class ParetoSet(SortedKeyList):
    ...
    def recluster(self):
        ...
        self.clustering = self.cluster_model.labels_
    ...
    def add(self, p):
        ...
        left = self.bisect_right(p)
        # if the new position of the value is not at the end and the point to its left is has a greater y value then add it
        if len(self) == 0: # Pareto set is empty
            is_pareto = True
        elif left != right: # There is another solution with same x value
            if self[right +1].y >= p.y:
                self.pop(right+1)
                is_pareto = True
            elif len(self) == right and self[right-1].y > p.y: # edge case
                is_pareto = True
            elif right == 0 or self[right].y > p.y:
            #if the point to its right has a greater or equal y value then remove that point
                if self[right+1].y >= p.y:
                    self.pop(right+1)
                    is_pareto = True
...
remove points proportionally from each cluster
        ...
        if len(self) > max_points and len(self) > 5:
            ...
            cluster_counts[:,1] = int(cluster_counts[:,1]/len(self.num_clusters)*num_points_remove)
            clustering_arr = np.array(self.clustering)
            for i in range(cluster_counts.shape[0]):
                ii = np.where(clustering_arr == i)[0]
                items_delete = np.random.choice(ii,cluster_counts[i,1])
                items_delete[::-1].sort()
                for item in items_delete:
                    del self.clustering[item]
                    self.pop(item)
