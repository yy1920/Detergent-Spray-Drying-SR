...
from .functions import _function_map, _Function, mul2,add2,sig1 as sigmoid
from .utils import _partition_estimators
from .utils import check_random_state
from .pareto import ParetoSet, Point
...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, PA, params):
    ...
    donor_indices = []
    if pareto_size > 0:
        #donor_indices = np.random.choice(np.arange(n_programs),pareto_size)
        random_state = np.random.RandomState(seeds[0]) #Create random state for the pareto crossover tournament
        for i in range(pareto_size):
            donor, donor_index = _tournament()
            donor_indices.append([donor, donor_index])
    ...
    while i < (n_programs):
        ...
        if parents is None:
            ...
            donor_program = donor_indices[i][0]
            program, removed, remains = PA[pareto_index].data.crossover(donor_program.program,random_state)                                                                           
            genome = {'method': 'Pareto Mutation',                                                                                                                       
                      'parent_idx': donor_indices[i][1],                                                                                                             
    ...
    for i in range(len(programs)):
        program = programs[i]
        accepted = PA.add(Point(x=program.raw_fitness_, y=program.complexity(), data=program))
#        print(program.complexity(),program.raw_fitness_,accepted)
    PA.prune()
#    print(PA.to_array())
    return programs
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if isinstance(self, TransformerMixin):
            ...
            if self._metric.greater_is_better:
                self._program = self.PA[-1].data
            else:
                self._program = self.PA[0].data
        ...
        a = clf.intercept_
        self._program.program.insert(0,mul2)
        self._program.program.insert(0,add2)
        self._program.program.append(b)
        self._program.program.append(a)
        self.train_X = X
        self.train_y = y
        return self
...
class SymbolicRegressor(BaseSymbolic, RegressorMixin):
    ...
    def stacked_predict(self, X):
        ...
        num_predictors = min(len(self.PA),3)
        
        result = np.zeros((X.shape[0],num_predictors))
        for i in range(num_predictors):
            result[:,i] = self.PA[i].data.execute(X)
        return np.average(result, axis=1)
        

    def stacked_predict2(self, X,meta="ridge"):
        X = check_array(X)
        _, n_features = X.shape
        if self.n_features_in_ != n_features:
            raise ValueError('Number of features of the model must match the '
        ...
        for i in range(len(self.PA)):
            using_predictors.append(self.PA[i].data)

        result = np.zeros((self.train_X.shape[0],len(using_predictors)))
        for j in range(len(using_predictors)):
             result[:,i] = using_predictors[i].execute(self.train_X)
        metric = 10000000
        clf = Ridge(alpha=1.0)
        if meta == "mean":
            metric = self.score(self.train_y,np.average(result,axis=1))
        elif meta == "median":
            metric = self.score(self.train_y,np.median(result,axis=1))
        else:
            clf.fit(result, self.train_y.reshape(-1,1))
            metric = self.score(self.train_y,clf.predict(result))
        discarded_predictors = []
        best_mod = clf
        while keep_going:
            result = np.zeros((self.train_X.shape[0],len(using_predictors)))
            keep_going = False
            for i in range(len(using_predictors)):
                discarded_predictors.append(using_predictors.pop(i))
                
                for j in range(len(using_predictors)):
                    result[:,i] = using_predictors[i].execute(self.train_X)
                avg_rmse =10000000
                if meta == "mean":
                    avg_rmse = self.score(self.train_y,np.average(result,axis = 1))
                elif meta == "median":
                    avg_rmse = self.score(self.train_y,np.median(result,axis = 1))
                else:
                    clf = Ridge(alpha=1.0)
                    clf.fit(result, self.train_y.reshape(-1,1))
                    avg_rmse = self.score(self.train_y,clf.predict(result))
                if metric < avg_rmse:
                    using_predictors.insert(i,discarded_predictors.pop())
                else:
                    best_mod = clf
                    keep_going = True
                    break
            
        result = np.zeros((X.shape[0],len(using_predictors)))
        
        for i in range(len(using_predictors)):
            result[:,i] = using_predictors[i].execute(X)
        if meta == "mean":
            return np.average(result, axis=1)
        elif meta == "median":
            return np.median(result,axis = 1)
        return best_mod.predict(result)
