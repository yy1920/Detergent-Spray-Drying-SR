...
from .functions import _Function, add2,mul2,sqr1,exp1
...
class _Program(object):
    ...
    '''
    def complexity_helper(self,remaining):
        complexity = 0
        node = remaining.pop(0)
        
        if isinstance(node, _Function):
            new_complexity, remaining = self.complexity_helper(remaining)
            if node.name in {"add","sub"}:
                ...
                new_complexity, remaining = self.complexity_helper(remaining)
                complexity *= new_complexity
            elif node.name in {"sqr"}:
                
                complexity = new_complexity**2
            elif node.name in {"sqrt"}:
                
                complexity = new_complexity**3
            elif node.name in {"sin", "cos", "log", "exp"}:
                
                complexity  = 2**new_complexity
            else:
                complexity = 1
        else:
            complexity = 1
    ...
    def complexity(self):
        # Function computes complexity of this solution.
        return len(self.program)
    ...
    def complexity2(self):
        ...
        for node in self.program:
            if isinstance(node,_Function):
                if node.name in {"add","sub"}:
                    #self.complexity_array.append(add2)
                    complexity +=2
                elif node.name in {"mul","div"}:
                    #self.complexity_array.append(mul2)
                    complexity +=4
                elif node.name in {"sqr","sqrt"}:
                    #self.complexity_array.append(sqr1)
                    complexity +=6
                elif node.name in {"sin", "cos", "log", "exp"}:
                   # self.complexity_array.append(exp1)
                    complexity +=8
                else:
                    #self.complexity_array.append(1)
                    complexity +=1
            else:
                #self.complexity_array.append(1)
                complexity+=1
    ...
    '''
    def complexity2(self):
        X = np.ones((1,self.n_features))
        if len(self.complexity_array) == 0:
            self.complexity_init()
        ...
        for node in self.complexity_array:
            if isinstance(node, _Function):
                apply_stack.append([node])
            else:
                # Lazily evaluate later                                                                                                                                                                             
