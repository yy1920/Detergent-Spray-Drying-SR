...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, PA, params):
    ...
    if pareto_size > 0:
        ...
        donor_indices[::-1].sort()
    # Loop breeds Pareto solutions with random solutions from populations
        '''
        for i in range(pareto_size):
            random_state = check_random_state(seeds[donor_indices[i]])
            donor_program = parents.pop(donor_indices[i])
            program, removed, remains = PA[i][2].crossover(donor_program.program,random_state)
            genome = {'method': 'Pareto Mutation',
                              'parent_idx': donor_indices[i],
    ...
    for i in range(n_programs):
        ...
        if parents is None:
            program = None
            genome = None
        elif i in donor_indices:
            donor_program = parents[i]
            program, removed, remains = PA[d_count][2].crossover(donor_program.program,random_state)                                                                           
            genome = {'method': 'Pareto Mutation',                                                                                                                       
                      'parent_idx': donor_indices[d_count],                                                                                                             
                      'parent_nodes': removed}
            d_count +=1
        else:   
    ...
    if parents == None:
        return programs
    for parent in PA.get_pareto_points():
        parent = parent[2]
        max_samples += 1
        n_samples += 1
        donor, donor_index = _tournament()
        ...
        if sample_weight is None:                                                                                                                                           
            ...
            curr_sample_weight = sample_weight.copy()                                                                                                                       
        oob_sample_weight = curr_sample_weight.copy()                                                                                                                       
        print("max samples: ",max_samples, n_samples)
        ...
        programs.append(program)
    print("end of evolve ",len(programs))
    '''
    for i in range(len(programs)):
        # INSERT THE PARETO STUFF HERE!!!
        #print(program.complexity(),program.raw_fitness_)
        PA.add(Point(x=program.complexity(), y=program.raw_fitness_, data=program))
    PA.prune()
    return programs
